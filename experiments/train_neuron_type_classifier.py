"""Train a gradient-boosted classifier for excitatory/inhibitory neuron type.

This secondary experiment asks whether node-level statistics derived from the
activity-matched SNN weights contain enough information to distinguish excitatory
from inhibitory source neurons. Labels are derived from the sign of outgoing
ground-truth synaptic weights, while features summarize learned incoming and
outgoing weights across burst-specific connectivity estimates.

The script also supports grouped train/test sweeps and saves per-network
probabilities, feature importances, ROC/PR metrics, and summary heatmaps.
"""
...
OUTPUT_DIR = PROJECT_ROOT / "ei_prob_matrices"
...
def resolve_npy_dir(folder_name: str, learned_base_dir: str | Path) -> Path:
    """Return the learned-connectivity directory for one statistical class."""
...
def load_all_connectivity_runs_from_one_folder(
    ...
    """Load all learned-matrix stacks for one statistical network class."""
...
def load_ground_truth_matrices_for_folder(
    ...
    """Load signed ground-truth adjacency matrices for one statistical class."""
...
def load_all_connectivity_runs_multiple_folders(
    ...
    """Load learned and signed ground-truth matrices across statistical classes."""
...
def load_default_experiment_data() -> dict[str, dict[str, object]]:
    """Load the default statistical classes used in the neuron-type study."""
    all_data = load_all_connectivity_runs_multiple_folders(
        folder_names=NETWORK_STATISTICS,
        learned_base_dir=LEARNED_CONNECTIVITY_DIR,
        ground_truth_base_dir=GROUND_TRUTH_DIR,
        npy_pattern=CONNECTIVITY_FILE_GLOB,
    )
...
def _validate_network_inputs(
    ...
    """Validate matrix dimensions before node-level feature construction."""
...
def _build_node_features_for_one_network(
    ...
    *,
    ...
    """Construct node-level learned-weight features and neuron-type labels."""
...
def _reconstruct_neuron_vector(
    ...
    *,
    ...
    """Map predictions for labeled neurons back to a length-N vector."""
...
def _safe_binary_metrics(
    ...
    """Compute classification and ranking metrics with single-class safeguards."""
    metrics = {
...
def _numeric_series_without_nan(df: pd.DataFrame, column: str) -> pd.Series:
    """Return a numeric metric series after coercion and NaN removal."""
...
def save_test_prediction_arrays(
    ...
    *,
    ...
    """Save per-network neuron-type probabilities and predictions as NumPy arrays."""
...
def build_test_network_auc_summary(
    ...
    """Summarize ROC/PR performance independently for each held-out network."""
...
def save_combination_outputs(
    ...
    *,
    ...
    """Persist metrics, predictions, feature importances, and split metadata for one sweep cell."""
...
def create_auc_heatmap(
    ...
    *,
    ...
    """Create and save a train/test-count heatmap for one evaluation metric."""
...
def train_gbt_across_networks(
    ...
    *,
    ...
    """Fit and evaluate the neuron-type GBT on network-level train/test splits."""
...
def flatten_loaded_data_by_group(
    ...
    """Flatten class-organized network data while retaining provenance metadata."""
    all_matrices_lists: list[np.ndarray] = []
    all_signed_A_t: list[np.ndarray] = []
...
def make_grouped_train_test_split(
    ...
    *,
    ...
    """Create reproducible train/test indices independently within each network class."""
...
def train_gbt_across_grouped_networks(
    ...
    *,
    ...
    """Create a grouped split and train the neuron-type classifier."""
...
def run_all_train_test_combinations(
    ...
    *,
    ...
    """Evaluate all valid per-class train/test-count combinations and save summaries."""

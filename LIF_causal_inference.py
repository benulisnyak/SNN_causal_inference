...
nest_burst_detection_window_ms = 50.0
...
def load_burst_samples(data_file_path: str | Path, data_dtres: float):
    ...
    burst_samples, burst_meta = extract_bursts_from_spiketrain(
        ...
        nest_window_ms=nest_burst_detection_window_ms,
...
class SurrogateLIFNetwork(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Keep the same neuron / synapse parameters as your current code.
        self.V_rest_mV = -65.0
...
#restrict candidate presynaptic neurons to the recent model history and make learning stronger for more recent spikes.
presynaptic_history_window_bins = 6

#optional gradient diagnostics for debugging stalls near the end of a sample
...
selected_sample_idx = None  # set to an integer index to train only one sample

#Dataset-level skipping. When True, any target dataset whose normalized
#connectivity_matrices_<dataset_suffix>.npy file already exists in LIFoutput_files is skipped before loading or training that dataset.
...
def train_one_dataset(data_file_path: str | Path):
    burst_samples, burst_meta = load_burst_samples(data_file_path=data_file_path, data_dtres=data_dtres)

    first_sample = burst_samples[0]
    N = int(first_sample.shape[1])
    W_base = make_random_W(N=N, p=p, G=G, seed=train_seed, device=sim_device, dtype=dtype)
    ...
    save_dir = Path(input_path) / "LIFoutput_files"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "The folder is resolved relative to <input_path>/fdata/ unless you provide an absolute path "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "legacy '_inhOff' suffix. This overrides enable_skip_trained_samples=False."
        ),
    )
    parser.add_argument(
        "--force-retrain",
        ...
        help=(
            ...
            "already exists. This overrides enable_skip_trained_samples=True."
        ),
    )
...
def main(reference_target_filename: str, *, skip_trained_samples: bool | None = None):
    configure_global_reproducibility(seed)
